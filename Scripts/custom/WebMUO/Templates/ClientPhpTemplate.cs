...
{
    public class ClientPhpTemplate : BasePhpTemplate
    {
        private readonly MUOTemplateWriter.TemplateConfig _config;

        public ClientPhpTemplate(string serverName, string port, MUOTemplateWriter.TemplateConfig config)
        ...
        {
            var template = new ClientPhpTemplate(config.ServerName, config.Port, config);
        ...
        private string GenerateTemplate()
        {
            ...
            AppendPhpHeader(sb, "client_errors.log", true, true, _config);

            // Load settings from sitesettings.json
            sb.AppendLine("<?php");
            sb.AppendLine("$activePage = 'client';");
            ...
            sb.AppendLine("  <title><?php echo htmlspecialchars($jsonData['Shard']['ServerName'] ?? 'Unknown Server'); ?> Client</title>");
            ...
            sb.AppendLine("    .content-section-client h2, .content-section-client p, .content-section-client a, .content-section-client small, .content-header h1, .content-header p { color: var(--text-color) !important; }");
            // Layout styles to match Updates page
            sb.AppendLine("    .content-wrapper-client { display: flex; flex: 1; overflow: hidden; position: relative; margin-top: 60px; margin-bottom: 60px; }");
            sb.AppendLine("    .main-section-client { flex: 0 0 100%; display: flex; flex-direction: column; margin: 0; width: 100%; min-height: calc(100vh - 120px); padding: 0; }");
            sb.AppendLine("    .content-section-client { display: flex; flex-direction: column; width: 100%; max-width: 90vw; margin: 0 auto; height: 100%; text-align: center; align-items: center; }");
            ...
            // Style the client info section
            sb.AppendLine("    .client-info-container { margin-top: 10px; width: 100%; max-width: 90vw; background: rgba(0, 0, 0, 0.7); padding: 15px; border: 1px solid #333; margin-left: auto; margin-right: auto; text-align: center; }");
            sb.AppendLine("    .client-info-container p { margin: 0 0 20px; font-size: 1.2em; }");
            sb.AppendLine("    .client-info-container a.download-button { display: block; padding: 8px 16px; background: <?php echo htmlspecialchars($settings['client_download_button_color']); ?>; color: #000; text-decoration: none; border-radius: 5px; margin: 10px auto; max-width: 200px; width: 100%; text-align: center; font-weight: bold; }");
            sb.AppendLine("    .client-info-container a.download-button:hover { opacity: 0.8; }");
            sb.AppendLine("    .client-info-container .connection-details { margin: 10px 0; font-size: 1.1em; }");
            sb.AppendLine("    .client-info-container small { font-size: 0.9em; display: block; margin-top: 10px; }");
            sb.AppendLine("    @media (max-width: 768px) {");
            sb.AppendLine("      .content-section-client { max-width: 100%; padding: 10px; }");
            ...
            sb.AppendLine("      .client-info-container { max-width: 100%; padding: 10px; }");
            sb.AppendLine("      .client-info-container p { font-size: 1em; }");
            sb.AppendLine("      .client-info-container a.download-button { max-width: 150px; }");
            sb.AppendLine("      .client-info-container .connection-details { font-size: 1em; }");
            sb.AppendLine("    }");
            sb.AppendLine("  </style>");
            sb.AppendLine("</head>");
            ...
            sb.AppendLine("      <div class='content-wrapper-client'>");
            sb.AppendLine("        <div class='main-section-client'>");
            sb.AppendLine("          <div class='content-section-client'>");
            sb.AppendLine("            <div class='content-header'>");
            sb.AppendLine("              <h1><?php echo htmlspecialchars($jsonData['Shard']['ServerName'] ?? 'Unknown Server'); ?> Client</h1>");
            sb.AppendLine("              <p>Join the shard with our custom client.</p>");
            sb.AppendLine("            </div>");
            sb.AppendLine("            <div class='client-info-container'>");
            sb.AppendLine("              <p><?php echo htmlspecialchars($settings['client_text_above_button']); ?></p>");
            sb.AppendLine("              <a href='<?php echo htmlspecialchars($settings['client_download_url']); ?>' class='download-button'>Download Client</a>");
            sb.AppendLine("              <p class='connection-details'>Host: <?php echo htmlspecialchars($settings['client_host'] ?? 'localhost'); ?></p>");
            sb.AppendLine("              <p class='connection-details'>Port: <?php echo htmlspecialchars($settings['client_port'] ?? '2593'); ?></p>");
            sb.AppendLine("              <small><?php echo htmlspecialchars($settings['client_text_below_button']); ?></small>");
            sb.AppendLine("            </div>");
            sb.AppendLine("          </div>");
            sb.AppendLine("        </div>");
            sb.AppendLine("      </div>");
