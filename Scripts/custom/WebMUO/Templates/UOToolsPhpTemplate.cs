...
{
    public class UOToolsPhpTemplate : BasePhpTemplate
    {
        private readonly MUOTemplateWriter.TemplateConfig _config;

        public UOToolsPhpTemplate(string serverName, string port, MUOTemplateWriter.TemplateConfig config)
        ...
        {
            var template = new UOToolsPhpTemplate(config.ServerName, config.Port, config);
        ...
        private string GenerateTemplate()
        {
            ...
            AppendPhpHeader(sb, "uotools_errors.log", true, true, _config);

            // Load settings and tools data
            sb.AppendLine("<?php");
            sb.AppendLine("$activePage = 'uotools';");
            ...
            sb.AppendLine("    <title><?php echo htmlspecialchars($jsonData['Shard']['ServerName'] ?? 'Unknown Server'); ?> UO Tools</title>");
            ...
            sb.AppendLine("        .content-section-uotools h1, .content-section-uotools table th, .content-section-uotools table td, .content-section-uotools table td a, .content-header h1, .content-header p { color: var(--text-color) !important; }");
            sb.AppendLine("        .content-section-uotools table td a:hover { color: #99eeff; }");
            // Layout styles to match Updates page
            sb.AppendLine("        .content-wrapper-uotools { display: flex; flex: 1; overflow: hidden; position: relative; margin-top: 60px; margin-bottom: 60px; }");
            sb.AppendLine("        .main-section-uotools { flex: 0 0 100%; display: flex; flex-direction: column; margin: 0; width: 100%; min-height: calc(100vh - 120px); padding: 0; }");
            sb.AppendLine("        .content-section-uotools { display: flex; flex-direction: column; width: 100%; max-width: 90vw; margin: 0 auto; height: 100%; }");
            ...
            sb.AppendLine("        .table-container-uotools { margin-top: 10px; width: 100%; max-width: 90vw; background: rgba(0, 0, 0, 0.7); padding: 15px; border: 1px solid #333; overflow-y: auto; flex-grow: 1; max-height: calc(100vh - 220px); box-sizing: border-box; margin-left: auto; margin-right: auto; }");
            sb.AppendLine("        .table-container-uotools table { width: 100%; border-collapse: collapse; table-layout: auto; }");
            sb.AppendLine("        .table-container-uotools th, .table-container-uotools td { padding: 10px; text-align: left; border-bottom: 1px solid #333; }");
            sb.AppendLine("        .table-container-uotools th { background: #222; }");
            sb.AppendLine("        @media (max-width: 768px) {");
            sb.AppendLine("            .content-section-uotools { padding: 10px; max-width: 100%; }");
            ...
            sb.AppendLine("            .table-container-uotools { padding: 10px; max-width: 100%; max-height: calc(100vh - 200px); }");
            sb.AppendLine("            .table-container-uotools th, .table-container-uotools td { padding: 8px; font-size: 0.9em; }");
            sb.AppendLine("        }");
            sb.AppendLine("    </style>");
            sb.AppendLine("</head>");
            ...
            // Unique content wrapper for UO Tools page with scrollable table
            sb.AppendLine("            <div class=\"content-wrapper-uotools\">");
            sb.AppendLine("                <div class=\"main-section-uotools\">");
            sb.AppendLine("                    <div class=\"content-section-uotools table-section-uotools\">");
            sb.AppendLine("                        <div class=\"content-header\">");
            sb.AppendLine("                            <h1><?php echo htmlspecialchars($jsonData['Shard']['ServerName'] ?? 'Unknown Server'); ?> UO Tools</h1>");
            sb.AppendLine("                            <p>Helpful tools for Ultima Online gameplay.</p>");
            sb.AppendLine("                        </div>");
            sb.AppendLine("                        <div class=\"table-container-uotools\">");
            sb.AppendLine("                            <table>");
            sb.AppendLine("                                <thead><tr><th>Tool Name</th><th>Description</th></tr></thead>");
            sb.AppendLine("                                <tbody>");
            sb.AppendLine("                                    <?php");
            sb.AppendLine("                                    if (!empty($tools)) {");
            sb.AppendLine("                                        foreach ($tools as $tool) {");
            sb.AppendLine("                                            echo '<tr>';");
            sb.AppendLine("                                            echo '<td><a href=\"' . htmlspecialchars($tool['url']) . '\" target=\"_blank\">' . htmlspecialchars($tool['name']) . '</a></td>';");
            sb.AppendLine("                                            echo '<td>' . htmlspecialchars($tool['description']) . '</td>';");
            sb.AppendLine("                                            echo '</tr>';");
            sb.AppendLine("                                        }");
            sb.AppendLine("                                    } else {");
            sb.AppendLine("                                        echo '<tr><td colspan=\"2\">No tools available.</td></tr>';");
            sb.AppendLine("                                    }");
            sb.AppendLine("                                    ?>");
            sb.AppendLine("                                </tbody>");
            sb.AppendLine("                            </table>");
